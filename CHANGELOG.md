...
- **`docs/FAQ.md`** — fresh user-facing troubleshooting and orientation. Covers install bypass on each OS, no-sound triage, plugin-not-in-DAW, MIDI input expectations, the Follow toggle, preset locations (per OS), recovering overwritten factory presets, "render JUST one beep" workaround, complete uninstall paths (per OS), `Cmd+Z` scope on Randomize, bypass-during-playback behaviour, pre-release labelling, where to report bugs.
- **Native macOS screen-top menu bar.** On macOS standalone, b33p now installs its menu via `juce::MenuBarModel::setMacMainMenu` so File / Edit / Help appear in the screen-top menu bar where Mac users expect them. Plugins (host owns the screen menu) and Windows / Linux keep the embedded in-window menu bar. The selection happens at construction via `JUCE_MAC + JUCEApplicationBase::isStandaloneApp`; the embedded strip's 24 px reservation is dropped when the screen menu is in use, so the window is shorter on macOS standalone than before.
- **30-second quickstart** at the top of both `README.md` and `docs/USAGE.md` — four numbered steps that get a fresh user from launch to "I made a beep" without scrolling. The README quickstart also links the new FAQ.
- **A/B / Copy group outline** in the Master strip. The A / B toggle and Copy buttons now sit inside a thin rounded outline so they read as one A/B-compare unit rather than three peers of Undo / Redo / preset-arrow controls on the same row.
- **README pre-release explanation.** Tagged releases marked "Pre-release" on the GitHub Releases page no longer leave users wondering if the binary is unstable — the README now explains that "Pre-release" means "not yet 1.0" and that every tagged build still passes the CI gates.
- **Randomize buttons can never go silent.** Pressing Randomize used to occasionally land on a combination (filter closed, amp sustain ~0, Mod FX wet swallows the dry, …) that produced no sound — the user heard nothing and assumed the app was broken. There's now an audibility guard: after every bulk-randomize the synth offline-renders the audition for each touched lane, and if the result is below an audibility floor (~ −40 dBFS) it re-rolls the level-critical parameters up to five times, falling back to a known-audible safe patch if that still doesn't recover. Randomize Pattern's clip placement is unchanged. Wired into Master "Randomize All" and Pattern "Randomize Params"; per-knob dice are intentionally left alone (single-parameter exploration). 64-seed regression test guarantees the rule going forward.
- **Output safety limiter.** A memoryless soft-clip safety stage on the summed output prevents the four lane voices + MIDI pool from hard-clipping when a hot patch or a dense pattern would otherwise push the mix past full-scale. Identity below a −3 dBFS knee, exponential soft-knee asymptoting to a −0.3 dBFS ceiling (C1-continuous at the knee, so no audible kink). Clean signals are untouched; only would-be-clipping peaks get gently rounded.
- **Live modulation glow on every modulatable knob.** Knobs whose parameter is being modulated during playback paint a soft accent halo whose intensity tracks the live modulation. Mod-matrix LFO routings light their destination knob (cutoff, drive, gain, etc.); the amp envelope lights the gain knob over the course of a note; the pitch envelope lights the base-pitch knob whenever the curve deflects. Combined via max so multiple sources show through. (See the *also* below for the gate — glow is intentionally absent when the synth is silent.)
- **Mod FX activity glow surfaces the warble at a glance.** When the Mod FX type is Chorus, Flanger, or Phaser (the modes with internal LFOs), the section's three knobs pulse at the effect's actual rate during playback — so the user can SEE which section is producing a warble they hear. None / Reverb / Delay don't glow.
- **Randomize change-flash.** When Randomize fires, each rolled knob briefly pulses a brighter accent ring (~ 0.8 s ease-out decay) so the user can see which knobs moved. Per-knob dice flashes just that knob. Distinct from the modulation glow (paints at the value-arc radius; modulation glow paints further out) so the two cleanly coexist on the same knob.
- **Live playhead on the Amp Envelope visualizer.** A vertical accent line walks through the painted A-D-S-R curve during playback, tracking the current envelope stage + elapsed time. Sustain parks the playhead at the start of the sustain region (sustain holds a constant level — a walking playhead there would be motion for its own sake). Idle: no playhead, no repaints.
- **"Randomize Pattern" button** in the pattern toolbar scatters fresh random clips across all four lanes in one undoable step.

### Changed

- **Distortion and Bitcrush are now 4x oversampled to reduce aliasing.** Both nonlinear per-lane effect stages — the Distortion waveshaper and the Bitcrush sample-rate/bit-depth reducer — now run their nonlinearity at 4x the host sample rate (`juce::dsp::Oversampling`, half-band polyphase IIR) before decimating back down, so the harmonics/images they generate are pushed above the audible band instead of folding back into it. Bitcrush's sample-and-hold rate is scaled against the oversampled tick rate, so a given Rate setting still crushes to the same audible reduction ratio as before — a preset saved at 48 kHz sounds the same. This adds a small, fixed amount of algorithmic latency -- 12 samples at 48 kHz (6 samples each for Distortion and Bitcrush), the same at every supported buffer size and sample rate -- reported via `setLatencySamples` so host PDC compensates automatically; there is no new user-facing control.
- **Master "Randomize All" renamed to "Randomize Voice"** to match the rest of the Voice strip vocabulary. Scope is unchanged (every unlocked parameter on the currently-selected lane); the undoable transaction name now reads "Randomize Voice (lane N)" and the tooltip is correspondingly explicit. This is the third name on this button across the 0.2.x line ("Randomize Lane" → "Randomize All" → "Randomize Voice") — the new name is the keeper.
- **`(Lane N)` suffix is back on every voice-editor section** (Amp Envelope, Filter, Effects, Mod FX, Modulation, Master). The 0.2.0 release had collapsed the suffix down to the Oscillator section only as a noise reduction, but end-user testing showed it left people wondering "which lane am I editing right now?" mid-session. Each section now carries the suffix alongside the existing accent-strip colour, so the answer to that question is in-place at every section the user looks at. The accent strip stays as the at-a-glance signal; the suffix is the answer-the-question signal.
- **Unsaved-changes prompt copy is consistent across every entry point.** File ▸ New, File ▸ Open, File ▸ Open Recent, File ▸ Load Preset, and dropped `.beep` files all now show the same neutral "Save changes to *filename*?" message. The previous wording varied by entry point (load-preset said one thing, open-recent another), which read like several different features had landed on the same dialog.
- **Pitch Envelope empty-state hint** now appears whenever the curve is effectively flat — the default state is two boundary points both at 0 semitones, which previously drew as the zero baseline and looked indistinguishable from "nothing's here." The hint reads `Click anywhere to add a pitch point. Drag to shape; right-click a point to delete.` and disappears the moment any point has a non-zero semitone value or a third point is added.
- **USAGE.md Mod FX label table** now matches the abbreviations actually shown in the app (Damp / Fback / Mix) instead of the prior internal long names, so the table stops contradicting the running app for Reverb / Delay rows.
- **USAGE.md "Lane menu" reference fixed** — the right-click hint at line 141 now correctly points at `Edit ▸ Lane` (the previous prose had survived from before the menu was demoted).
- **Randomize controls renamed for clarity.** Master "Randomize Lane" → **"Randomize All"** (same scope: every unlocked parameter on the currently-selected lane). Pattern "Randomize All" → **"Randomize Params"**, and rescoped to roll synth parameters only for lanes that currently hold clips — randomizing a silent lane's voice is wasted motion. Empty pattern falls back to rolling every lane so the button is never dead.
...
- **Step through presets without opening the browser.** The Master section now has `<` / `>` arrows (with the current preset's name between them) that load the previous / next preset in your collection, wrapping around. Loading is undoable and prompts before discarding unsaved changes, same as the browser.

### Fixed

- **No UI element animates while the synth is silent (accessibility).** Earlier cuts of the modulation glow + matrix LFO indicators kept pulsing when no audio was playing — motion when idle is an accessibility problem (motion sensitivity, attention drift) and gives a false signal that something's happening. Every timer-driven repaint now gates by playback state: knob glow, Mod FX activity, mod-matrix indicators, and the AmpEnvelope playhead all freeze the moment the selected lane's voice goes idle, with one final settle-repaint on the playing → idle edge so visuals don't stick mid-pulse. Verified: at idle, four screen captures across three seconds are pixel-identical.
...
- **User presets now save to the house location on macOS.** `PresetManager` built its per-user presets folder from `juce::File::userApplicationDataDirectory`, which on macOS resolves to plain `~/Library` (not `~/Library/Application Support` as this file's own code comment and every doc claimed) — presets were actually landing at `~/Library/b33p/Presets`, an undocumented path nothing else pointed at. Presets now save to `~/Library/Audio/Presets/ZQ SFX/b33p`, the house convention for company-keyed user-data folders (`../CLAUDE.md` section 2, already used by LFlOw and Broken). Windows (`%APPDATA%\b33p\Presets`) and Linux (`~/.config/b33p/Presets`) were already correct and are unchanged. A one-time, copy-not-move migration (`Source/State/PresetMigration.h`, same idea as `Project_Worldizer/Source/Plugin/PresetMigration.h`) copies any presets found at the old macOS location into the new one on first launch — the old files are never deleted or moved, an existing file at the new location is never overwritten, and a marker file makes the migration idempotent (and lets a preset the user later deletes from the new folder stay deleted rather than reappearing from the old one). Covered by `Tests/State/PresetMigrationTests.cpp` (temp directories only, never the real per-user folders). Docs updated: `docs/FAQ.md`, `docs/USAGE.md`.
- **Every knob's dice/lock button pair, the pattern grid's Mute/Solo buttons, and the BPM stepper's +/- buttons now meet the house 22 px hit-area floor (accessibility).** `LabeledSlider`'s dice/lock row was 20 px tall everywhere a knob shows randomizer buttons (Oscillator, Amp Envelope, Filter, Effects, Mod FX, Modulation) — 2 px under the floor in `../CLAUDE.md` section 6. Fixing the row height surfaced a second, worse bug in `ModulationSection`: the LFO Rate knob's layout budget was already so tight that the accessibility floor pushed its rotary slider to a silently-collapsed 0 px tall and its own dice/lock row to 16 px, in both the shipped build and this one before the fix — the knob was simply invisible and unclickable. `PatternGrid`'s Mute/Solo buttons were 14 px tall (an unrelated pre-existing gap in the same row-height family), and `PatternSection`'s BPM `+`/`-` stepper (a `Slider::IncDecButtons`-style child) was 18 px wide. All four are now fixed: `LabeledSlider`'s dice/lock row is 22 px; `ModulationSection`'s LFO row grew to give the Rate knob a real, visible size (absorbed by `MainComponent`'s `kModulationRowHeight`, which only lengthens the already-scrolling voice-editor viewport, never clips); `PatternGrid`'s lane-label strip drops its vertical inset so Mute/Solo fill the full 22 px row; `PatternSection`'s BPM slider gained 12 px so its inc/dec buttons split to 24 px each. New gate: `b33p_ui_snapshot --hit-audit [width height]` constructs the real editor and walks every visible Button/ComboBox/Slider (including custom clickables built on them, e.g. `IconButton`, `zqsfx::ui::LogoMark`) printing any under 22 px in either dimension; wired into `ctest` at both the editor's declared 1000x600 minimum and its default construction size (`ui_hit_audit_min_size` / `ui_hit_audit_default_size`). 0 violations across 96 audited controls at both sizes after the fix; before/after renders show no overlap or clipping at either size.
- **Bitcrush and Distortion's 4x oversampling now processes whole blocks instead of one sample at a time.** `OversampledBitcrush`/`OversampledDistortion` called `juce::dsp::Oversampling`'s up/down filter pair once per single sample (`Voice::processSample()` ran the full chain, including the oversampler, sample-by-sample), paying its per-call overhead on every sample instead of once per block. Both classes gained a `processBlock(float*, int)` that batches the oversampler call across a whole chunk (preallocated up to `kMaxOversampledBlockSize` = 2048 in `prepare()`, chunking internally beyond that — no audio-thread allocation); `processSample()` is now defined as `processBlock(&x, 1)` so it stays exactly identical to the old per-sample behaviour for every existing caller and test. `Voice` splits into `generateCore()` (oscillator → amp envelope → filter, still sample-accurate for pattern trigger/noteOff timing) and `applyEffectsBlock()` (the batched bitcrush → distortion → modEffect → gain tail); `B33pProcessor::processBlock` now runs a per-chunk pass — generate each sample (pattern timing unchanged), batch the effects tail per voice, then mix + limit + write — instead of one fully-interleaved per-sample pass. Bitcrush/distortion state intentionally carries across triggers already (Voice.h), so this changes nothing about *what* gets computed, only how many samples are batched into each oversampler call. Verified bit-for-bit (within 1e-6) identical output to the old per-sample path at 64/100/1024-sample blocks and 44.1/48/96 kHz (`Tests/DSP/OversamplingBlockProcessingTests.cpp`, `Tests/DSP/VoiceBlockProcessingTests.cpp`, including a mid-block-retrigger case), and `Tests/State/LatencyTests.cpp`'s existing block-size/sample-rate invariance test still passes unchanged (the oversampler's filter design, and therefore its reported latency, doesn't depend on how many samples are batched per call). Benchmark (16 voices, 30 s render, Release build): ~2.3-2.9x faster at realistic host buffer sizes (256-2048 samples), ~1.9-2.7x even at a small 64-sample buffer; see the fix's verification notes for the full before/after table.
...
- **File ▸ Restore Factory Presets...** rewrites the four shipped presets (FM Bell, Resonant Stab, Delay Pad, Ring Mod Robot) to their originals — recovering any you saved over — behind a confirmation. Your own presets are untouched.
- **Edit ▸ Lane ▸ Copy / Paste voice (clipboard)** — copy a dialed-in lane's whole voice (every parameter + its wavetable slots) to the system clipboard and paste it into another lane, or another b33p instance entirely. Paste is one undoable step.
...
- **Master level meter** gains a peak-hold marker (a bright tick that holds the recent maximum ~1.5 s, then eases down) and a latching **clip indicator** (a red cap at the right end that lights for ~1.5 s whenever the output hits 0 dBFS, so a brief overload isn't missed between frames).
- **Pitch Envelope** canvas now shows axis reference: faint vertical gridlines at the quarter points of the note duration and `+12 / 0 / -12` semitone labels down the left edge, so the curve reads against concrete pitch values.
- The **selected lane** now persists in saved projects and DAW sessions, and the **editor window size** persists across DAW close / reopen (stored in the plugin's DAW state, deliberately *not* in shared `.beep` files — window size isn't portable patch data).
- Numeric value readouts under each knob (and the drag-time popup bubble) now use a **monospaced** face, so digits no longer shift left/right as a value ticks during a drag.
- VST3 / AU plugin **subcategory** is now declared explicitly (`Instrument | Synth` / `kAudioUnitType_MusicDevice`) so host plugin pickers that filter by subcategory list b33p correctly.
- **Knob interactions.** Every rotary now supports the synth standards: hold **Shift** before dragging for fine adjustment, **mouse-wheel / two-finger scroll** to step the value, **double-click** to reset to the parameter default, and a **right-click menu** (Enter value… / Reset to default) — so typing an exact value is finally discoverable.
- Custom flat dark **LookAndFeel**. Rotary knobs drop JUCE's default line indicator for a flat body + thin value arc + dot, and the arc takes the **selected lane's accent colour** (blue / green / amber / pink) so the knobs shift colour with the section strips when you switch lanes. Comboboxes are flat with a hairline border + accent caret; buttons are flat rounded; popup menus and dialogs match the charcoal palette. Modulation-amount sliders now render bipolar (fill grows from the centre). Installed process-wide so the editor and its dialogs share one look.
...
- Loading a preset is now a single undoable step — Cmd+Z (or the in-plugin Undo button) restores the previous patch instead of losing it.
- Saving a preset under a name that already exists now asks for confirmation before overwriting, instead of silently replacing the existing preset.
- Standalone window now fits the screen on launch. The voice editor was a six-row vertical stack ~1338 px tall — taller than a 1080p display's usable height — so the pattern grid opened off the bottom of the screen. The three full-width-but-short rows (Mod FX, Modulation, Pitch Env) are now column-packed: Mod FX sits beside Effects + Master, and Modulation sits beside Pitch Env, bringing the default window to ~1007 px. The standalone window also clamps to the display's usable area on launch (and stays resizable), so it never opens partly off-screen. The editor additionally gained resize limits (min 1000×600, max 3200×2200) so it can't be dragged down to an unreadable sliver or ballooned past sane bounds.
