...
- [ ] **Schema version constant lags content (discovered 2026-05-26).** `ProjectState::kCurrentVersion = 12`, but `save()` already writes the v13 A/B-compare node and `migrate()` has a 12→13 step — so current files are stamped `version=12` while carrying v13 content. It works by luck (load tolerates the missing-then-present node), but the constant should be `13`. Bumping it is a `.beep` format change (older builds would then reject files stamped 13), so flagging rather than silently changing — decide whether to correct the constant. Not blocking; the new P7 `selected_lane` field is version-independent (tolerant default).
