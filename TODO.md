...
Driven by user feedback re-establishing two long-standing project rules: (1) every randomize button must produce an audible patch (the original "no silent randomize" rule from early development), and (2) the user must be able to see what's actually shaping the sound at a glance so they can troubleshoot. Recorded as project memories `rule-randomize-must-be-audible`, `principle-nothing-hidden-see-at-a-glance`, and `rule-no-motion-when-idle`.
