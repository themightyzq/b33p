...
- [ ] **Clear the residual clang-tidy backlog, then flip the CI job to blocking.** The first run surfaced 314 warnings, but 285 were `performance-enum-size` (a debatable micro-opt re-counted per TU — now disabled) and the 2 `bugprone-inc-dec-in-conditions` hits (the note-off countdowns at `PatternRenderer.cpp:88` and `B33pProcessor.cpp:1110`) are fixed. What's left is ~8 minor, non-bug perf findings — each safe (clang-tidy only flags params it proves are used as a plain const ref), but a couple touch `std::function` params so they want a per-site read before changing: `performance-unnecessary-value-param` at `ParameterLayout.cpp:22` (`range`), `ProjectFileManager.cpp:175` (`onComplete`), `ProjectState.cpp:290` (`patternNode`), `UndoableActions.h:29/85/130` (`editorToRefresh`), `InspectorStrip.cpp:243` (`mutator`); and `performance-no-automatic-move` at `PresetManager.cpp:84` (`destination`). Fix them, drop `continue-on-error` from the `clang-tidy` job, and set `WarningsAsErrors` in `.clang-tidy`.
