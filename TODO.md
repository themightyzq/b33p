...
- [ ] **Triage the clang-tidy backlog, then flip the CI job to blocking.** Review the first `clang-tidy` run's findings (the job log lists counts by check), fix or per-line-suppress them, drop `continue-on-error` from the job, and set `WarningsAsErrors` in `.clang-tidy`. Bounded follow-up to the rail above.
