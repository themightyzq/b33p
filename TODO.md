...
- [~] **REVIEW.md Pass 2 P18 — master meter feedback.** Added a **peak-hold** marker (bright tick at the recent max, holds ~1.5 s then eases down) and a latching **clip indicator** (red cap at the right end, lights ~1.5 s on any 0 dBFS sample). Both ride the existing 30 Hz timer + `meterBounds` paint. **Deferred:** the -12 / -6 / -3 / 0 dBFS headroom tick marks — the meter is only 6 px tall; legible dB ticks would need a taller meter / Master-section re-layout.
