...
- [ ] **Local clang-tidy helper so the blocking gate is runnable pre-push.** Right now clang-tidy can't run locally: `CMakeLists` forces a Universal macOS build, so `compile_commands.json` has two `-cc1` jobs per file and clang-tidy errors with "expected exactly one compiler job". A blocking gate you can't run locally means every lint slip is a red-CI round-trip. Add a single-arch compile-commands path (e.g. a `tools/clang-tidy.sh` that configures a throwaway `-DCMAKE_OSX_ARCHITECTURES=<host>` build with `CMAKE_EXPORT_COMPILE_COMMANDS=ON` and runs clang-tidy over `Source/`), or document the override. Until then, read findings from the (now-fixed) CI job log.
