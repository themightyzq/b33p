...
namespace B33p
{
    ...
    //
    // Also paints a live playhead (P31) during playback: a vertical
    // accent line at the x position corresponding to the selected
    // lane's amp envelope stage + time-in-stage, gated by
    // isSelectedLaneVoiceActive (idle = no playhead, no timer ticks).
    ...
    {
    public:
        explicit AmpEnvelopeVisualizer(B33pProcessor& processor);
    ...
    private:
        ...
        int currentLane { 0 };
        // Edge-tracks playback for the playhead repaint (same pattern
        // as ModulationSection's indicator strips: one final repaint
        // on playing→idle so the playhead clears cleanly).
